...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    synchronized public void startLeaderElection() {
        ...
        for (QuorumServer p : getView().values()) {
            if (p.id == myid) {
                //获得当前 zkserver myid 对应的ip地址： 1 ->myQuorumAddr=192.168.13.102
                myQuorumAddr = p.addr;
                break;
            }
        }
        ...
        // 根据 electionType 创建对应的选举算法,electionType 默 认值为 3.可以在配置文件中动态配置
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            if(listener != null){
                //  listener 实现了线程，所以在 run 方法中可以看到构建ServerSocket 的请求，
